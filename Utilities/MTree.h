...
//	};
    ...
    eval(DataMap &dataMap, shared_ptr<ParametersTable> PT, vector<vector<double>> &vectorData) = 0;
...
public:
    ...
    virtual vector<double>
    eval(DataMap &dataMap, shared_ptr<ParametersTable> PT, vector<vector<double>> &vectorData) override {
...
inline shared_ptr<Abstract_MTree> stringToMTree(string formula) {
        ...
        if (isdigit(formula[index]) || formula[index] == '.') {
            ...
            while (formula[index] != ']') {
                ...
                if (index > formulaSize) {
                    cout << "  In stringToMTree() :: while converting " << formula
            ...
            currString = "";
                   ...
                   memchr(ops, formula[index], sizeof(ops))) { // if char at index is an opperation (+,-,*,/, etc)...
            //////cout << "found op: " << formula[index] << endl;
            if (op == 'x') {
                op = formula[index];
                //////cout << "    setting op" << op << endl;
            } else if (op != formula[index]) {
                cout << "  In stringToMTree() :: while converting " << formula << ", found two diffrent operators! \""
                     << op << "\" and " << formula[index]
                     << " in the same parentheses.\n  Please add parentheses to resolve oder of operations.\n  Exiting"
                     << endl;
                exit(1);
            }
            if (branches.size() == 0) {
                cout << "  In stringToMTree() :: while converting " << formula << ", found op \"" << op
    ...
    if (op == 'x') { // no op, this must be a single term
        if (branches.size() > 1) {
            cout << "  In MTree, more then one argument provided with out an operator in formula: " << formula
                 << ". exiting." << endl;
            exit(1);
        }
        return branches[0];

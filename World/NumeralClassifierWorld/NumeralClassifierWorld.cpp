...
shared_ptr<ParameterLink<int>> NumeralClassifierWorld::defaulttestsPreWorldEvalPL = Parameters::register_parameter(
        "WORLD_NUMERALCLASSIFIER-testsPreWorldEval", 5,
        "number of values each brain attempts to evaluate in a world evaluation");
shared_ptr<ParameterLink<int>> NumeralClassifierWorld::defaultWorldUpdatesPL = Parameters::register_parameter(
        "WORLD_NUMERALCLASSIFIER-WorldUpdates", 100, "number of world updates brain has to evaluate each value");
shared_ptr<ParameterLink<int>> NumeralClassifierWorld::defaultRetinaTypePL = Parameters::register_parameter(
        "WORLD_NUMERALCLASSIFIER-retinaType", 3, "1 = center only, 2 = 3 across, 3 = 3x3, 4 = 5x5, 5 = 7x7");
shared_ptr<ParameterLink<string>> NumeralClassifierWorld::numeralDataFileNamePL = Parameters::register_parameter(
        "WORLD_NUMERALCLASSIFIER-dataFileName",
        (string) "World/NumeralClassifierWorld/mnist.train.discrete.28x28-only100", "name of file with numeral data");

NumeralClassifierWorld::NumeralClassifierWorld(shared_ptr<ParametersTable> _PT) :
        AbstractWorld(_PT) {
    worldUpdates = (PT == nullptr) ? defaultWorldUpdatesPL->lookup() : PT->lookupInt(
            "WORLD_NUMERALCLASSIFIER-WorldUpdates");
    testsPreWorldEval = (PT == nullptr) ? defaulttestsPreWorldEvalPL->lookup() : PT->lookupInt(
            "WORLD_NUMERALCLASSIFIER-testsPreWorldEval");
    retinaType = (PT == nullptr) ? defaultRetinaTypePL->lookup() : PT->lookupInt(
            "WORLD_NUMERALCLASSIFIER-retinaType");  //1 = center only, 2 = 3 across, 3 = 3x3, 4 = 5x5, 5 = 7x7
    numeralDataFileName = (PT == nullptr) ? numeralDataFileNamePL->lookup() : PT->lookupString(
            "WORLD_NUMERALCLASSIFIER-dataFileName");
    ...
    cout << "NumeralClassifierWorld requires brains with at least " << inputNodesCount + outputNodesCount
         << " nodes.\n";
    if (inputNodesCount == 0) {
        ...
        cout << "    " << outputNodesCount << " Outputs\t nodes " << inputNodesCount << " to "
             << inputNodesCount + outputNodesCount - 1 << "\n";
    }
    ...
    if (FILE.is_open()) {  // if the file named by configFileName can be opened
        while (getline(FILE,
                       rawLine)) {  // keep loading one line from the file at a time into "line" until we get to the end of the file
        ...
        for (int worldUpdate = 0; worldUpdate < worldUpdates; worldUpdate++) {
            ...
            for (int i = 0; i < retinaSensors; i++) {  // fill first nodes with food values at here location
                ...
                if (checkX >= 0 && checkX < 28 && checkY >= 0 && checkY < 28) {  // if we are on the image
                    org->brain->setInput(nodesAssignmentCounter++,
                                         numeralData[numeralPick][(whichNumeral * 28 * 28) + (checkY * 28) + (checkX)]);
...
//	cout << endl;
    ...
    for (int i = 0; i < 10; i++) {
        ...
        (counts[i] < testsPreWorldEval) ? val = (double) incorrect[i] / ((double) testsPreWorldEval - counts[i])
                                        : val = 0;
        org->dataMap.Append(temp_name, val);
        org->dataMap.setOutputBehavior(temp_name, DataMap::AVE);
    }

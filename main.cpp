...
int main(int argc, const char *argv[]) {
    ...
    bool saveFiles = Parameters::initializeParameters(argc,
                                                      argv);  // loads command line and configFile values into registered parameters
    ...
    if (saveFiles) { // if saveFiles (save settings files) is set
        ...
        Parameters::saveSettingsFiles(maxLineLength, commentIndent, {"*"},
    ...
    for (auto NS : groupNameSpaces) {
        ...
        aveFileColumns.insert(aveFileColumns.end(), population[0]->genome->aveFileColumns.begin(),
                              population[0]->genome->aveFileColumns.end());
        aveFileColumns.insert(aveFileColumns.end(), population[0]->brain->aveFileColumns.begin(),
                              population[0]->brain->aveFileColumns.end());
...
//		cout << endl;
        ...
        cout << "Group name: " << NS << "\n  " << popSize << " organisms with " << PT->lookupString("GENOME-genomeType")
             << "<" << PT->lookupString("GENOME-sitesType") << "> genomes and " << PT->lookupString("BRAIN-brainType")
             << " brains.\n  Optimizer: "
             << PT->lookupString("OPTIMIZER-optimizer") << "\n  Archivist: "
             << PT->lookupString("ARCHIVIST-outputMethod") << endl;
        cout << endl;
        // end of report
    }
    ...
    if (Global::modePL->lookup() == "run") {
        ...
        while (!groups[defaultGroup]->archivist->finished) {
            ...
            world->evaluate(groups, false, false,
                            AbstractWorld::debugPL->lookup());  // evaluate each organism in the population using a World
        ...
        groups[defaultGroup]->population[0]->genome->loadGenomeFile(Global::visualizePopulationFilePL->lookup(),
                                                                    testGenomes);
        ...
        if (IDs[0] == -1) { // visualize last
            ...
            for (auto ID : IDs) {
                if (ID != -2) {
                    ...
                    if (!found) {
                        cout << "ERROR: in visualize mode, can not find genome with ID " << ID << " in file: "
                             << Global::visualizePopulationFilePL->lookup() << ".\n  Exiting." << endl;
                        exit(1);
                    }
                } else {
                    padPopulation = true;
            ...
            if (foundCount < (int) IDs.size()) {
                cout << "WARRNING: in visualize mode " << (int) IDs.size() - foundCount
                     << " genomes specified in Global::visualizeOrgID could not be found." << endl;
            }
        ...
        if (padPopulation) {
            ...
            if ((int) testGenomes.size() < Global::popSizePL->lookup()) {
                cout << "  Population size is larger then the number of genomes in the file. Padding population with "
                     << Global::popSizePL->lookup() - (int) testGenomes.size() << " extra copies." << endl;
            ...
            if ((int) testGenomes.size() > Global::popSizePL->lookup()) {
                cout
                        << "  Population size is smaller then the number of genomes in the file. deleting genomes from head of population."
                        << endl;
                testGenomes.erase(testGenomes.begin(),
                                  testGenomes.begin() + ((int) testGenomes.size() - Global::popSizePL->lookup()));
            }
        }
        ...
        for (auto g : testGenomes) {
            ...
            if (inUseIDs.find(g->dataMap.GetIntVector("ID")[0]) != inUseIDs.end()) {
                ...
                newOrg->ID = (int) g->dataMap.GetAverage(
                        "ID"); // this is the first copy of this genome, so it get's it's own value!
            }
            ...
            testPopulation.push_back(
                    newOrg);  // add a new org to population using progenitors template and a new random genome
        }

        shared_ptr<Group> testGroup = make_shared<Group>(testPopulation, groups[defaultGroup]->optimizer,
                                                         groups[defaultGroup]->archivist);
        ...
        for (auto o : testGroup->population) {
            cout << "  organism with ID: " << o->genome->dataMap.GetAverage("ID") << " generated score: "
                 << o->dataMap.GetAverage("score") << " " << endl;
        }
    } else {
        cout << "\n\nERROR: Unrecognized mode set in configuration!\n  \"" << Global::modePL->lookup()
             << "\" is not defined.\n\nExiting.\n" << endl;
        exit(1);
    }

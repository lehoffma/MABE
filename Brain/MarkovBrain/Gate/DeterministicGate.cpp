...
shared_ptr<ParameterLink<string>> DeterministicGate::IO_RangesPL = Parameters::register_parameter(
        ...
        "range of number of inputs and outputs (min inputs-max inputs,min outputs-max outputs)");
...
void DeterministicGate::update(vector<double> &nodes, vector<double> &nextNodes) {
    // converts the input values into an index (true indicates to reverse order)
    int input = vectorToBitToInt(nodes, inputs, true);

...
shared_ptr<ParameterLink<string>> DefaultArchivist::Arch_outputMethodStrPL = Parameters::register_parameter(
        "ARCHIVIST-outputMethod", (string) "This_string_is_set_by_modules.h",
        "This_string_is_set_by_modules.h");  // string parameter for outputMethod;
////// ARCHIVIST-outputMethod is actually set by Modules.h //////

shared_ptr<ParameterLink<string>> DefaultArchivist::Arch_realtimeSequencePL = Parameters::register_parameter(
        "ARCHIVIST_DEFAULT-realtimeSequence", (string) ":10",
...
shared_ptr<ParameterLink<string>> DefaultArchivist::SS_Arch_dataSequencePL = Parameters::register_parameter(
        "ARCHIVIST_DEFAULT-snapshotDataSequence", (string) ":100",
...
shared_ptr<ParameterLink<string>> DefaultArchivist::SS_Arch_organismSequencePL = Parameters::register_parameter(
        "ARCHIVIST_DEFAULT-snapshotOrganismSequence", (string) ":1000",
...
shared_ptr<ParameterLink<bool>> DefaultArchivist::Arch_writePopFilePL = Parameters::register_parameter(
        "ARCHIVIST_DEFAULT-writePopFile", true, "Save data to average file?");
shared_ptr<ParameterLink<bool>> DefaultArchivist::Arch_writeMaxFilePL = Parameters::register_parameter(
        "ARCHIVIST_DEFAULT-writeMaxFile", true, "Save data to Max file?");
shared_ptr<ParameterLink<string>> DefaultArchivist::Arch_DefaultPopFileColumnNamesPL = Parameters::register_parameter(
        "ARCHIVIST_DEFAULT-popFileColumns", (string) "[]",
        "data to be saved into average file (must be values that can generate an average). If empty, MABE will try to figure it out");

shared_ptr<ParameterLink<string>> DefaultArchivist::Arch_FilePrefixPL = Parameters::register_parameter(
        "ARCHIVIST_DEFAULT-filePrefix", (string) "NONE",
        "prefix for files saved by this archivst. \"NONE\" indicates no prefix.");
shared_ptr<ParameterLink<bool>> DefaultArchivist::SS_Arch_writeDataFilesPL = Parameters::register_parameter(
...
shared_ptr<ParameterLink<bool>> DefaultArchivist::SS_Arch_writeOrganismsFilesPL = Parameters::register_parameter(
        "ARCHIVIST_DEFAULT-writeSnapshotOrganismsFiles", false,
        "if true, snapshot organism files will be written (with all organisms for entire population)");
...
DefaultArchivist::DefaultArchivist(shared_ptr<ParametersTable> _PT, string _groupPrefix) :
    ...
    DataFilePrefix = (groupPrefix == "") ? "snapshot_data" : groupPrefix.substr(0, groupPrefix.size() - 2) + "__" +
                                                             "snapshot_data";
    DataFilePrefix = (Arch_FilePrefixPL->get(PT) == "NONE") ? DataFilePrefix : Arch_FilePrefixPL->get(PT) +
                                                                               DataFilePrefix;

    OrganismFilePrefix = (groupPrefix == "") ? "snapshot_organisms" : groupPrefix.substr(0, groupPrefix.size() - 2) +
                                                                      "__" + "snapshot_organisms";
    OrganismFilePrefix = (Arch_FilePrefixPL->get(PT) == "NONE") ? OrganismFilePrefix : Arch_FilePrefixPL->get(PT) +
                                                                                       OrganismFilePrefix;
...
DefaultArchivist::DefaultArchivist(vector<string> popFileColumns, shared_ptr<Abstract_MTree> _maxFormula,
    ...
    for (auto key : DefaultPopFileColumns) {
        ...
        if (seperatorCharPos != string::npos && DataMap::knownOutputBehaviors.find(key.substr(seperatorCharPos + 1)) !=
            ...
            if (uniqueColumnNameToOutputBehaviors.find(key.substr(0, seperatorCharPos)) ==
                uniqueColumnNameToOutputBehaviors.end()) { // if key not in map
                uniqueColumnNameToOutputBehaviors[key.substr(0,
                                                             seperatorCharPos)] = DataMap::knownOutputBehaviors[key.substr(
                        seperatorCharPos + 1)];
...
void DefaultArchivist::saveSnapshotData(vector<shared_ptr<Organism>> population) {
    ...
    for (auto org : population) {
        ...
        if (org->snapshotAncestors.size() != 1 ||
            ...
            while (parentCheckList.size() > 0) {
                ...
                if (find(saveList.begin(), saveList.end(), parent) !=
                    ...
                    if (parent->timeOfBirth < minBirthTime || (parent->snapshotAncestors.size() == 1 &&
            ...
            */
            ...
            if (org->timeOfBirth >= Global::update) { // if this is a new org...
                cout
                        << "  WARRNING :: in DefaultArchivist::saveSnapshotData(), found new org (age < 1) with self as ancestor (with ID: "
        ...
        if (org->timeOfBirth < Global::update || saveNewOrgs) {
            ...
            //cleanup to avoid too much memory consumption
        ...
        for (auto org : population) {  // we don't need to worry about tracking parents or lineage, so we clear out this data every generation.
            if (!writeSnapshotDataFiles) {
                org->parents.clear();
                       ...
                       org->snapshotAncestors.end()) { // if ancestors contains self, then this org has been saved and it's ancestor list has been collapsed
                org->parents.clear();
...
bool DefaultArchivist::isDataUpdate(int checkUpdate) {
    ...
    check = check ||
            find(realtimeDataSequence.begin(), realtimeDataSequence.end(), checkUpdate) != realtimeDataSequence.end();
    return check;
}
...
bool DefaultArchivist::isOrganismUpdate(int checkUpdate) {
    ...
    bool check = find(realtimeOrganismSequence.begin(), realtimeOrganismSequence.end(), checkUpdate) !=
                 realtimeOrganismSequence.end();
    return check;
}

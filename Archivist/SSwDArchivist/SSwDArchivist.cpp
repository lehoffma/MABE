...
shared_ptr<ParameterLink<string>> SSwDArchivist::SSwD_Arch_dataSequenceStrPL = Parameters::register_parameter(
        "ARCHIVIST_SSWD-dataSequence", (string) ":100",
...
shared_ptr<ParameterLink<string>> SSwDArchivist::SSwD_Arch_organismSequenceStrPL = Parameters::register_parameter(
        "ARCHIVIST_SSWD-organismSequence", (string) ":1000",
...
shared_ptr<ParameterLink<int>> SSwDArchivist::SSwD_Arch_dataDelayPL = Parameters::register_parameter(
        "ARCHIVIST_SSWD-dataDelay", 10,
        "when using Snap Shot with Delay output Method, how long is the delay before saving data");
shared_ptr<ParameterLink<int>> SSwDArchivist::SSwD_Arch_organismDelayPL = Parameters::register_parameter(
        "ARCHIVIST_SSWD-organismDelay", 10,
        "when using Snap Shot with Delay output Method, how long is the delay before saving organisms ");
shared_ptr<ParameterLink<int>> SSwDArchivist::SSwD_Arch_cleanupIntervalPL = Parameters::register_parameter(
        "ARCHIVIST_SSWD-cleanupInterval", 100, "How often to cleanup old checkpoints");
shared_ptr<ParameterLink<string>> SSwDArchivist::SSwD_Arch_FilePrefixPL = Parameters::register_parameter(
        "ARCHIVIST_SSWD-filePrefix", (string) "NONE",
        "prefix for files saved by this archivst. \"NONE\" indicates no prefix.");
shared_ptr<ParameterLink<bool>> SSwDArchivist::SSwD_Arch_writeDataFilesPL = Parameters::register_parameter(
        "ARCHIVIST_SSWD-writeDataFiles", true, "if true, data files will be written");
shared_ptr<ParameterLink<bool>> SSwDArchivist::SSwD_Arch_writeOrganismFilesPL = Parameters::register_parameter(
        "ARCHIVIST_SSWD-writeOrganismFiles", true, "if true, genome files will be written");

SSwDArchivist::SSwDArchivist(vector<string> popFileColumns, shared_ptr<Abstract_MTree> _maxFormula,
                             shared_ptr<ParametersTable> _PT, string _groupPrefix) :
    ...
    DataFilePrefix = (groupPrefix == "") ? "SSwD_data" : groupPrefix.substr(0, groupPrefix.size() - 2) + "__" +
                                                         "SSwD_data";
    DataFilePrefix = (SSwD_Arch_FilePrefixPL->get(PT) == "NONE") ? DataFilePrefix : SSwD_Arch_FilePrefixPL->get(PT) +
                                                                                    DataFilePrefix;
    OrganismFilePrefix = (groupPrefix == "") ? "SSwD_organisms" : groupPrefix.substr(0, groupPrefix.size() - 2) + "__" +
                                                                  "SSwD_organisms";
    OrganismFilePrefix = (SSwD_Arch_FilePrefixPL->get(PT) == "NONE") ? OrganismFilePrefix :
...
void SSwDArchivist::cleanup() {

    {
        ...
        for (auto checkpoint : checkpoints) {  // for every checkpoint
            if (checkpoint.first < (Global::update - max(dataDelay,
                                                         organismDelay))) {  // if that checkpoint is older then the longest intervalDelay
                ...
                for (auto weakPtrToOrg : checkpoints[checkpoint.first]) {  // than for each element in that checkpoint
                    if (auto org = weakPtrToOrg.lock()) {  // if this ptr is still good
                        if ((!org->alive) && (org->timeOfDeath < (Global::update - max(dataDelay,
                                                                                       organismDelay)))) {  // and if the organism was dead before the current interesting data
                ...
                if (checkpointEmpty) {  // if all of the organisims in this checkpoint have been dead for a good long time...
                    ...
                    expiredCheckPoints.push_back(
                            checkpoint.first);  // add this checkpoint to a list of checkpoints to be deleted
                }
            }
        }
...
bool SSwDArchivist::archive(vector<shared_ptr<Organism>> population, int flush) {
    ...
    if (flush != 1) {
        ...
        if ((Global::update == realtimeSequence[realtimeSequenceIndex]) &&
            (flush == 0)) {  // do not write files on flush - these organisms have not been evaluated!
        ...
        if ((Global::update == realtimeDataSequence[realtimeDataSeqIndex]) && (flush == 0) &&
            writeSnapshotDataFiles) {  // do not write files on flush - these organisms have not been evaluated!
            saveSnapshotData(population);
            if (realtimeDataSeqIndex + 1 < (int) realtimeDataSequence.size()) {
                realtimeDataSeqIndex++;
            }
        }
        if ((Global::update == realtimeOrganismSequence[realtimeOrganismSeqIndex]) && (flush == 0) &&
            writeSnapshotGenomeFiles) {  // do not write files on flush - these organisms have not been evaluated!
        ...
        if ((Global::update == nextOrganismCheckPoint && writeOrganismFiles) ||
            (Global::update == nextDataCheckPoint && writeDataFiles)) {  // if we are at a data or genome interval...
            ...
            for (auto org : population) {  // add the current population to checkPointTracker
                if (saveNewOrgs || org->timeOfBirth <
                ...
                if (Global::update == nextDataCheckPoint && Global::update <= Global::updatesPL->get()) {
                    ...
                    if (org->ancestors.size() != 1 || org->ancestors.find(org->ID) == org->ancestors.end()) {
                        ...
                        while (parentCheckList.size() > 0) {
                            ...
                            if (find(saveList.begin(), saveList.end(), parent) !=
                                saveList.end()) { // if this parent is being saved, they will serve as an ancestor
                                org->ancestors.insert(parent->ID);
                            } else { // this parent is not being saved
                                if (parent->timeOfBirth < minBirthTime || (parent->ancestors.size() == 1 &&
                        ...
                        */
                        ...
                        if (org->timeOfBirth >= Global::update) { // if this is a new org...
                            cout
                                    << "  WARRNING :: in SSwD::archive(), while adding to a snapshot data found new org (age < 1) with self as ancestor (with ID: "
                                    << org->ID << "... this will result in a new root to the phylogony tree!" << endl;
                            if (saveNewOrgs) {
                    ...
                    if (saveNewOrgs ||
                        org->timeOfBirth < Global::update) { // if this org is set up to be saved in this snapshot
                        ...
                        org->ancestors.insert(
                                org->ID);  // now that we have saved the ancestor data, set ancestors to self (so that others will inherit correctly)
            ...
            if (Global::update == nextOrganismCheckPoint && Global::update <=
                                                            Global::updatesPL->get()) {  // we have now made a genome checkpoint, advance nextGenomeCheckPoint to get ready for the next interval
            ...
            if (Global::update == nextDataCheckPoint && Global::update <=
                                                        Global::updatesPL->get()) {  // we have now made a data checkpoint, advance nextDataCheckPoint to get ready for the next interval
        ...
        if ((Global::update == nextOrganismWrite + organismDelay) && (nextOrganismWrite <= Global::updatesPL->get()) &&
            writeOrganismFiles) {  // now it's time to write genomes in the checkpoint at time nextGenomeWrite
            ...
            while (index < checkpoints[nextOrganismWrite].size()) {
                if (auto org = checkpoints[nextOrganismWrite][index].lock()) {  // this ptr is still good
                    ...
                    swap(checkpoints[nextOrganismWrite][index],
                         checkpoints[nextOrganismWrite].back());  // swap expired ptr to back of vector
                    checkpoints[nextOrganismWrite].pop_back();  // pop expired ptr from back of vector
                }
            }

            FileManager::closeFile(
                    organismFileName); // since this is a snapshot, we will not be writting to this file again.
        ...
        if ((Global::update == nextDataWrite + dataDelay) && (nextDataWrite <= Global::updatesPL->get()) &&
                ...
                while (!found) {  // check each org in checkPointTraker[nextDataWrite] until we find a valid org
                    if (auto temp_org = checkpoints[nextDataWrite][0].lock()) {  // this ptr is still good
                        ...
                        swap(checkpoints[nextDataWrite][0],
                             checkpoints[nextDataWrite].back());  // swap expired ptr to back of vector
            ...
            FileManager::closeFile(dataFileName);
    ...
    finished = finished || ((nextDataWrite > Global::updatesPL->get() || !(writeDataFiles)) &&
                            (nextOrganismWrite > Global::updatesPL->get() || !(writeOrganismFiles)) &&
    ...
    for (auto org : population) {  // we don't need to worry about tracking parents or lineage, so we clear out this data every generation.
        if (!writeSnapshotDataFiles && !writeDataFiles && !writeOrganismFiles) {
            org->parents.clear();
            //cout << "HERE?" << endl;
        } else if (org->snapshotAncestors.find(org->ID) != org->snapshotAncestors.end() &&
                   org->ancestors.find(org->ID) != org->ancestors.end() && (org->timeOfDeath < (Global::update -
                                                                                                max(dataDelay,
                                                                                                    organismDelay)))) { // if ancestors and snapshotAncestors contains self, then this org has been saved and it's ancestor list has been collapsed
            org->parents.clear();
    ...
    while (toCheck.size() > 0) {
        ...
        toCheck.pop_back();
        if ((org->timeOfBirth < minBirthTime) && (org->timeOfDeath < (Global::update - max(dataDelay,
                                                                                           organismDelay)))) { // no living org can be this orgs ancestor and this org died long enough ago that they can not be in a SSwD snapshot
            ...
            for (auto p : org->parents) { // we need to check parents (if any)
                if (checked.find(p) ==
                    checked.end()) { // if parent is not already in checked list (i.e. either checked or going to be)
                    toCheck.push_back(p);
                    checked.insert(org); // make a note, so we don't check twice
                }
            }
